...
export class WasmMatch {
     ...
     * @returns {string}
     */
    apply(player_id, command_json) {
        let deferred3_0;
        let deferred3_1;
        try {
            const ptr0 = passStringToWasm0(command_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
            const len0 = WASM_VECTOR_LEN;
            const ret = wasm.wasmmatch_apply(this.__wbg_ptr, player_id, ptr0, len0);
            var ptr2 = ret[0];
            var len2 = ret[1];
            if (ret[3]) {
                ptr2 = 0; len2 = 0;
     ...
     * @param {string} plugin_json
     * @param {bigint} seed
     */
    constructor(plugin_json, seed) {
        const ptr0 = passStringToWasm0(plugin_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.wasmmatch_new(ptr0, len0, seed);
        if (ret[2]) {
    ...
    snapshot_json() {
        ...
        let deferred2_1;
        try {
            const ret = wasm.wasmmatch_snapshot_json(this.__wbg_ptr);
            var ptr1 = ret[0];
            var len1 = ret[1];
            if (ret[3]) {
                ptr1 = 0; len1 = 0;
    ...
    state_hash() {
        ...
        let deferred1_1;
        try {
            const ret = wasm.wasmmatch_state_hash(this.__wbg_ptr);
            deferred1_0 = ret[0];
            deferred1_1 = ret[1];
...
if (Symbol.dispose) WasmMatch.prototype[Symbol.dispose] = WasmMatch.prototype.free;

/**
 * @returns {string}
 */
export function wasm_engine_version() {
    let deferred1_0;
    let deferred1_1;
    try {
        const ret = wasm.wasm_engine_version();
        deferred1_0 = ret[0];
        deferred1_1 = ret[1];
        return getStringFromWasm0(ret[0], ret[1]);
    } finally {
...
const WasmMatchFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_wasmmatch_free(ptr, 1));
